...
class MyanmarFoodDeepSeekEvaluator:
    def __init__(self):
        ...
        if not api_key:
            raise ValueError("OPENRAI_API_KEY not found in environment variables")
    ...
    def call_deepseek_api(self, prompt: str) -> str:
        """Call OPENAI API via OpenRouter and return response"""
        try:
...
"""
        ...
        try:
            ...
            import re as _re
            parsed_indices = [_re.sub(r"[^0-9]", "", tok) for tok in _re.split(r"[\s,]+", llm_response.strip()) if tok]
            parsed_indices = [int(x) for x in parsed_indices if x != ""]
...
def main():
    ...
    try:
        ...
        # Print summary
        print("\n" + "=" * 50)
        print("openai/gpt-4o EVALUATION SUMMARY")
        print("=" * 50)
